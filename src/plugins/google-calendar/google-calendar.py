...
class GoogleCalendar(BasePlugin):
    ...
    def fetch_calendar(self, calendar_url):
        try:
            if "calendar.google.com/calendar/embed" in calendar_url:
                parsed_url = urllib.parse.urlparse(calendar_url)
                query_params = urllib.parse.parse_qs(parsed_url.query)
                if 'src' in query_params:
                    calendar_id = query_params['src'][0]
                    calendar_url = f"https://calendar.google.com/calendar/ical/{urllib.parse.quote(calendar_id)}/public/basic.ics"
                    logger.info(f"Transformed Google Calendar URL to: {calendar_url}")
